...
def build_churn_agent(session_id: str, verbose: bool = True):
    print("--- Construindo ChurnAgent (simplificado) ---")
    tools = [
        Tool(
            name="getChurnOffers",
            # --- CORREÇÃO AQUI ---
            func=lambda *args, **kwargs: get_churn_offers(session_id=session_id),
            description="Use para buscar as ofertas de retenção disponíveis para o cliente."
        ),
        Tool(
            name="setContextOwner",
            func=lambda owner_string: set_context_owner(owner=owner_string, session_id=session_id),
            description="Use esta ferramenta no início da sua primeira interação para assumir o controle da conversa, definindo o contexto como `ChurnAgent`."
        )
    ]
